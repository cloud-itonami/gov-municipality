(ns gov-municipality.methods.social
  "social.cljc — gov-municipality DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from gov-municipality's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "gov-municipality が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over gov-municipality's own domain (Phase 0 regulatory & permitting actor. Coordinates building permit submission, inspections, and sign-off across jurisdictions per…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "gov-municipality R0: live social posting is Council Lv6+ + operator + "
